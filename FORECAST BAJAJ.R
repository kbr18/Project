...
print(adf.test(BajajClose))
modelfit <- auto.arima(BajajClose, lambda = "auto")
...
N = length(BajajClose)
n = 0.7*N
train = BajajClose[1:n, ]
test  = BajajClose[(n+1):N,  ]
...
#Dataset forecast upper first 5 values
fitarfima = autoarfima(data = BajajClose, ar.max = 2, ma.max = 2, 
...
#estimate model 
garch11closepricefit=ugarchfit(spec=garch11closeprice, data=BajajClose)
...
alpha <- 1.5^(-10)
hn <- length(BajajClose)/(alpha*(length(BajajClose)+30))

#Fitting nnetar
lambda <- BoxCox.lambda(BajajClose)
dnn_pred <- nnetar(BajajClose, size= hn, lambda = lambda)
