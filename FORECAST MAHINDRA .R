...
print(adf.test(MahindraClose))
modelfit <- auto.arima(MahindraClose, lambda = "auto")
...
N = length(MahindraClose)
n = 0.7*N
train = MahindraClose[1:n, ]
test  = MahindraClose[(n+1):N,  ]
...
#Dataset forecast upper first 5 values
fitarfima = autoarfima(data = MahindraClose, ar.max = 2, ma.max = 2, 
...
#estimate model 
garch11closepricefit=ugarchfit(spec=garch11closeprice, data=MahindraClose)
...
alpha <- 1.5^(-10)
hn <- length(MahindraClose)/(alpha*(length(MahindraClose)+30))

#Fitting nnetar
lambda <- BoxCox.lambda(MahindraClose)
dnn_pred <- nnetar(MahindraClose, size= hn, lambda = lambda)
