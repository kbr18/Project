...
print(adf.test(TataClose))
modelfit <- auto.arima(TataClose, lambda = "auto")
...
#Dataset forecast upper first 5 values
fitarfima = autoarfima(data = TataClose, ar.max = 2, ma.max = 2, 
...
#estimate model 
garch11closepricefit=ugarchfit(spec=garch11closeprice, data=TataClose)
...
alpha <- 1.5^(-10)
hn <- length(TataClose)/(alpha*(length(TataClose)+30))

#Fitting nnetar
lambda <- BoxCox.lambda(TataClose)
dnn_pred <- nnetar(TataClose, size= hn, lambda = lambda)
