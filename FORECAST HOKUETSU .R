...
print(adf.test(HokuetsuClose))
modelfit <- auto.arima(HokuetsuClose, lambda = "auto")
...
N = length(HokuetsuClose)
n = 0.7*N
train = HokuetsuClose[1:n, ]
test  = HokuetsuClose[(n+1):N,  ]
...
#Dataset forecast upper first 5 values
fitarfima = autoarfima(data = HokuetsuClose, ar.max = 2, ma.max = 2, 
...
#estimate model 
garch11closepricefit=ugarchfit(spec=garch11closeprice, data=HokuetsuClose)
...
alpha <- 1.5^(-10)
hn <- length(HokuetsuClose)/(alpha*(length(HokuetsuClose)+30))

#Fitting nnetar
lambda <- BoxCox.lambda(HokuetsuClose)
dnn_pred <- nnetar(HokuetsuClose, size= hn, lambda = lambda)
