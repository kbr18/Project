...
print(adf.test(LTClose))
modelfit <- auto.arima(LTClose, lambda = "auto")
...
#Dataset forecast upper first 5 values
fitarfima = autoarfima(data = LTClose, ar.max = 2, ma.max = 2, 
...
#estimate model 
garch11closepricefit=ugarchfit(spec=garch11closeprice, data=LTClose)
...
alpha <- 1.5^(-10)
hn <- length(LTClose)/(alpha*(length(LTClose)+30))

#Fitting nnetar
lambda <- BoxCox.lambda(LTClose)
dnn_pred <- nnetar(LTClose, size= hn, lambda = lambda)
