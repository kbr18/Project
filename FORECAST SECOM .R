...
print(adf.test(TokyoClose))
modelfit <- auto.arima(TokyoClose, lambda = "auto")
...
N = length(TokyoClose)
n = 0.7*N
train = TokyoClose[1:n, ]
test  = TokyoClose[(n+1):N,  ]
...
#Dataset forecast upper first 5 values
fitarfima = autoarfima(data = TokyoClose, ar.max = 2, ma.max = 2, 
...
#estimate model 
garch11closepricefit=ugarchfit(spec=garch11closeprice, data=TokyoClose)
...
alpha <- 1.5^(-10)
hn <- length(TokyoClose)/(alpha*(length(TokyoClose)+30))

#Fitting nnetar
lambda <- BoxCox.lambda(TokyoClose)
dnn_pred <- nnetar(TokyoClose, size= hn, lambda = lambda)
