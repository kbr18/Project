...
print(adf.test(AmadaClose))
modelfit <- auto.arima(AmadaClose, lambda = "auto")
...
N = length(AmadaClose)
n = 0.7*N
train = AmadaClose[1:n, ]
test  = AmadaClose[(n+1):N,  ]
...
#Dataset forecast upper first 5 values
fitarfima = autoarfima(data = AmadaClose, ar.max = 2, ma.max = 2, 
...
#estimate model 
garch11closepricefit=ugarchfit(spec=garch11closeprice, data=AmadaClose)
...
alpha <- 1.5^(-10)
hn <- length(AmadaClose)/(alpha*(length(AmadaClose)+30))

#Fitting nnetar
lambda <- BoxCox.lambda(AmadaClose)
dnn_pred <- nnetar(AmadaClose, size= hn, lambda = lambda)
