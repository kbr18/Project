...
print(adf.test(SecomClose))
modelfit <- auto.arima(SecomClose, lambda = "auto")
...
N = length(SecomClose)
n = 0.7*N
train = SecomClose[1:n, ]
test  = SecomClose[(n+1):N,  ]
...
#Dataset forecast upper first 5 values
fitarfima = autoarfima(data = SecomClose, ar.max = 2, ma.max = 2, 
...
#estimate model 
garch11closepricefit=ugarchfit(spec=garch11closeprice, data=SecomClose)
...
alpha <- 1.5^(-10)
hn <- length(SecomClose)/(alpha*(length(SecomClose)+30))

#Fitting nnetar
lambda <- BoxCox.lambda(SecomClose)
dnn_pred <- nnetar(SecomClose, size= hn, lambda = lambda)
